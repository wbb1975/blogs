...
为了开始研究用户名字空间，我们将使用一个小程序， [demo_userns.c](https://lwn.net/Articles/539941/)，它在一个新的用户名字空间创建了一个子进程。子进程仅仅打印了其有效用户 ID 和组 ID和它的[权限](http://man7.org/linux/man-pages/man7/capabilities.7.html)，用一个非特权用户运行该程序将产生下面的输出：
```
...
```
这个程序的输出展示了一些有趣的细节。其中之一是指派给子进程的权限。字符串 "=ep" （由库函数 cap_to_text() 产生，它将权限集转换为字符串表示）指示子进程拥有被允许且有效的全部权限集，虽然该程序是由一个非特权用户运行的。当一个用户名字空间被创建时，该用户名字空间的首进程被授予该名字空间里的全部权限集。这允许在其它进程在该名字空间被创建之前，首金城可以在名字空间里执行任何需要的初始化。
...
还有一个重要的点需要指出来--它在上面的输出中未出现。虽然新进程唉进的用户名字空间里拥有全部的权限集，它在父名字空间里却没有权限。这无关 clone() 的进程的凭证和权限。特别地，即使根用户调用 clone （CLONE_NEWUSER），创建的资金撤股任然在父名字空间里没有权限。
